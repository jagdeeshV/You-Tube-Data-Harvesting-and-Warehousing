...
def extract_channel_data(channel_id, frm):
    try:
...
def extract_data(channel_id):
    channel_data = extract_channel_data(channel_id, 'Reg')
...
st.markdown("<h6 style='text-align: center; color: grey;'>Channel Store Count </h6>", unsafe_allow_html=True)

txt1 = "No. of Channels: "+ str(channel_count)+ "      Videos: "+ str(video_count)+ "     Comments: "+ str(comment_count)
html_str = f"""
<style>
p.a {{
  font: bold 16px Courier; text-align: center; color: grey; &nbsp: 
}}
</style>
<p class="a">{txt1}</p>
"""
